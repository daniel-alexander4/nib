...
- T06 — red proofs; `recorded` moves. *(PARTIAL 2026-08-25, v1.117.174 — four rows for the half that landed; `recorded` 105 → 109. Carry's two return checks are **owed**: their fixture drives a helper rather than the verb, because a hostile hop needs a hostile PEER, so a patch against the product would leave the check passing.)*
