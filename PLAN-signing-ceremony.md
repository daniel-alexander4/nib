...
### P08 — More than two parties: **lifecycle and delivery** *(**CLOSED** 2026-09-02, v1.117.332 — 18 slices; ledger `ledgers/2026-09-02-p08-acceptance.md`, every criterion split on `and`: **C15 NOT met for four** (C02, C06's expired and abandoned halves, C07, C14), three ⚠ *met at a lower tier than the criterion asks*, the rest ✓. C12 and C13 were fixed rather than recorded, gaining tier-6 clauses at S09. **The graduation pass over 131 rows checked each against the CODE and found three readers that had never existed** — all P08.S01, all aspirational when written; two properties turned out covered under other names, one is `/pending 364`. **`--lan -n 4` had been RED for four commits** and the contract had said so since four phases earlier for an unrelated defect, so the red was expected by anyone who ran it; fixed at S09 with `/pending 363` for what the fix narrows. `inventorycheck` counted 12 shipped slices where 17 had shipped, because it could not see a bolded status marker; fixed at S06 and it then blocked S09's own close. Cross-slice: every `PruneCeremony*` call site is inside `closeOutStores`, and the only `os.RemoveAll` under `~/nib` is `RemoveMirror` with its one caller — ADR-012 holds. Tiers 0–4 and 6 green, `-race` clean.)* **(added 2026-08-18 — split out of P07 at the sixth plan-review)**
...
#### P08.S02 — The contribution reaches disk before it reaches the wire *(D24 as amended, gaps #15 and #17; C01, C02, C03)* *(**done** 2026-09-02, v1.117.325 — C01, C02 and C03 all met. **C01's driver was built in a `/runpending` sweep (`/pending 323`(d)) and the blocker this marker carried was WRONG**: it said the kill-at-the-instant driver *"needs a fault seam this repo has deliberately refused in the product"*, and it needs none. `coSignExchange` calls `rd.Store` — which commits the mirror — and its caller writes the frame afterwards, so the window is the whole document transfer; its opening is announced on disk by the party itself, and its WIDTH is a function of the document, so the clause builds a 4.4 MB one. A kill is a `kill`. **Two corrections came out of running it.** The first cut killed on `document.pdf` appearing and the resumed party re-signed — which looked like C01 being violated and was the probe firing one file early: `WriteMirror` writes the record LAST and says so, so an uncommitted contribution is one the design says to re-sign. And passing `ceremony=` to the CONVENER did not fail cleanly — the convener holds no invitation for its own ceremony, so `carries()` was false and a `signs:false` convener silently signed, refused at the far end as a prefix mismatch naming a signer nobody expected (`/pending 356`). Probed red by removing the disk read-through: the resumed hop asks the party to sign again.)*
...
#### P08.S04 — Between hops: the arm, and the end states as protocol facts *(D16 amendment, D28; C05, C06, C07)* *(**done** 2026-09-02, v1.117.329 — C05's transport parity and C07 met at v1.117.248; C06 discharged through the two slices its bullet was SPLIT into. **The third bullet is where the reading matters.** Its expiry half went to S04a (done, v1.117.283) and its telling half to S04b (done, v1.117.287), with S05e giving the object a producer and a courier (v1.117.322) and S05g the round that carries it (v1.117.320). What the bullet asked for and did NOT get is its last clause — *"a decline at hop 3 then refuses hop 4 by name"*. **That is not outstanding work; it was struck at S04b with the reason written there**: a convener-signed termination *"cannot bind the convener, who can simply not mint one and is also the sole courier"*, so what it buys is **earliness, not enforcement**, and every consumer must read absence as UNKNOWN rather than live. A refusing arrival gate built on it would be exactly the dependency S04a's expiry rule was forbidden from taking. **Claim of absence, with the search**: `grep -rn "ReadTermination|readTerminationRaw|\.Ended\b" --include=*.go internal/server/ internal/p2p/` returns seven production hits, every one on the **sending** side — `rearmDeliveries`' skip at `delivery.go:517`, the round's payload choice at `:727`, and `checkDeliveredPayload`, which verifies a termination it was HANDED. `checkArrival` and `AdmitContribution` mention terminations nowhere. That is the designed state, not a gap, and it is recorded here because a bullet quietly unmet and a bullet deliberately struck look identical from outside.)*
...
#### P08.S05c — The arm becomes addressable, and the spoken check gets a key *(D22; C05, C08; the TRIPWIRE)* — **new, 2026-08-31, split out of S05** *(done 2026-09-01, v1.117.315 — 5 clauses met, 1 NOT EXERCISED (tier 4 needs S05d's caller). **The grill overturned two of the slice's five bullets**: clause 2's single collision predicate would have re-pointed the user's armed pill, and clause 3 named an arm key for a defect whose cause was ordering. One grill finding was itself wrong — the guard already required `status()` as a third site — and is corrected in place. **The review then found the slice's own defect class reintroduced through the teardown**: `disarm()` tore down ONE slot while its own new doc comment claimed it tore down every one, so a delivery arm survived shutdown holding the rendezvous server, the shared socket and a self-refreshing port-map lease. Also: `setReceived` took an `armKind` and never stored it (compiler happy, half the feature dead), and `armWindowFor` started doing 128 MiB-capable disk I/O under the mutex `status()` polls. **12 of 249 red-proof rows went stale on the refactor** and were re-recorded against their own defects; `arm-doors-disagree` named a defect that can no longer exist and was re-expressed one level down.)*
...
Tasks:
- T01 — `session` gains a second, named delivery-arm slot; per-arm state (`ln`/`cer`/`cerCancel`/`until`/`addr`) moves onto an `arm` value, machine-wide state (`notice`, `received`, `pending`, `verify`) stays on `session`. *(done — and the sticky fields needed MORE than staying put: both doors cleared them outright, so they carry the kind that wrote them and a door clears only its own.)*
- T02 — `collidesLocked(kind)` for the two doors and `armedLocked()` kept for `status()`; the structural guard gains `status()` as a third required site (finding (a)). *(done — **finding (a) was wrong about the guard**: it already required `status()` as a third site. The substance held; the guard now enumerates the doors from source and checks each predicate at the site that owns its question.)*
- T03 — `TestASecondArmCannotOrphanALiveCeremony` re-expressed from *"must fail"* to *"must not overwrite"*, plus a test that an unrelated arm leaves a live delivery arm's notice and received record intact. *(done — plus a third test the REVIEW forced: an unconditional disarm must empty every slot.)*
...
- T05 — the delivery arm's bound from `Expires` + grace, floored at the interactive window, with what it tolerates named (finding (d)). *(done — `deliveryWindowFor` reads the party's own mirror, which `ReadMirror` verifies; an unreadable record yields the floor, never a long arm.)*
...
#### P08.S05e — The end state gets a producer and a courier *(D28, D34; C06's telling half)* — **new, 2026-08-31, split out of S05; SPLIT AGAIN 2026-09-01 at its deepdive** *(done 2026-09-02, v1.117.322 — 8 clauses, all met, none `not exercised`. **T04 was filed as harness-only and the harness was not the defect.** Its 409 assertion had been passing on the WRONG 409: no watcher answered the spoken check, `ErrVerificationTimedOut` fired, and `/api/session/initiate` answers 409 for that too — one of FIVE reasons it answers 409, four of which mean nobody refused anything. Driving the real decline then exposed the slice's own defect: the round walked the party that had just declined, whose machine prunes this ceremony's invitation when it refuses and which holds no mirror to check an attestation against, so the leg spent the whole 300 s `connectDeadline` — measured, `tried 0 address(es)` — on that round and on every re-run. **T05 skips it**, and the clause runs in 6 s instead of 360. **The review found the slice's own anchoring defect**: `checkDeliveredPayload` is a gate that REFUSES, and it verified an arriving attestation against the `record.json` sitting in the directory it was about to write into — the anchor `ReadTermination`'s doc forbids in as many words, and a planted matching pair verifies against itself into a durable false `Ended` that makes `rearmDeliveries` skip the ceremony forever. It also found the telling stating C06's item 2 **backwards** (*"the convener ended this proceeding"* when a party did), `markEndedBy` overwriting where its companion `WriteTermination` is write-once, and two absolutes in this slice's own new prose that are false when a vault is locked. **Two tiers were RED at HEAD and this slice closed them**: tier 2 since v1.117.320 (`deliveryOutcome` in neither table of the reader scan, so S05g closed over it) and tier 4 at N=3 since the same commit (S05g's recovery clause picks its victim dynamically and had its witness hard-coded to the same instance, so the witness WAS the victim). N=3 passes for the first time. 5 red proofs registered; tier 4 green at N=2, 3, 4 and 9.)*
