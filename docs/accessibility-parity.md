...
One number frames the checker rows. Nib checks **52 of the 106** rules veraPDF evaluates for PDF/UA-1, so
...
| PDF/UA identification in the file | Not described on the cited pages | Written only on Nib's own Markdown conversion, when the user chose its language, where veraPDF measures every construct Nib renders conformant. Never on the checker's say-so (it covers 52 of the 106 rules), never on an office conversion, and removed by any later change | `TestNibsOwnMarkdownConversionConformsAcrossEveryConstruct`, `TestPassingEveryClauseNibChecksIsNotConformanceAndTheDocsSaySo` | Partial |
