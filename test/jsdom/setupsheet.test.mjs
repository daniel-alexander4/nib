...
// ── /pending 506 — a binding to a document that has since closed ───────────────────────────────
//
// The binding was monotone one step too far: close the bound document and the pin kept naming an id
// the server no longer held. Every Convene then answered 409, and the sheet — reading `views` and
// finding nothing — said the document "is bound as soon as you come back", which it never was: the
// bind door refused to bind over a non-null id, and the only reset is the Convene button that RESUMES
// while parked. Two doors onto the same defect, both driven.
const line506 = () => doc.getElementById('cerSheetDoc');
const tabs506 = () => [...doc.querySelectorAll('#tabstrip .tab')];
async function open506(id, name, pages) {
  nextOpen = { id, name, path: '/tmp/nib-harness/' + name, canSave: true, signature: { state: 'unsigned' }, canUndo: false, canRedo: false };
  setNextDocument({ numPages: pages });
  doc.getElementById('pathInput').value = '/tmp/nib-harness/' + name;
  doc.getElementById('openGo').click();
  await settle();
}
async function closeTabNamed(name) {
  const tab = tabs506().find((t) => t.querySelector('.tabname').textContent === name);
  assert.ok(tab, `setup: no tab named ${name} to close`);
  tab.querySelector('.tabclose').click();
  await settle(30);
}

test('closing the bound document during the excursion rebinds on the way back, and says so', async () => {
  await open506('test-epoch:506a', 'bond.pdf', 4);
  assert.ok(tabs506().length >= 2, 'setup: two documents are not open, so there is nothing to close the bound one beside');
  doc.getElementById('ceremonyConveneBtn').click(); // fresh: binds the ACTIVE document, bond.pdf
  await settle();
  assert.match(line506().textContent, /bond\.pdf/, 'setup: the fresh open did not bind bond.pdf');

  doc.getElementById('cerSeeDoc').click();
  await settle();
  await closeTabNamed('bond.pdf');
  assert.ok(!tabs506().some((t) => t.querySelector('.tabname').textContent === 'bond.pdf'), 'setup: bond.pdf is still open');

  doc.getElementById('cerBackToSetup').click();
  await settle();
  assert.doesNotMatch(line506().textContent, /bond\.pdf/, 'the sheet still names a document that is closed');
  assert.match(line506().textContent, /This ceremony will be built from/,
    'the sheet came back bound to nothing while a document is open — the dead pin was kept, so every Convene answers 409');
  doc.getElementById('cerConveneCancel').click();
  await settle();
});

test('Convene with its bound document closed behind the sheet rebinds, says so, and convenes nothing', async () => {
  await open506('test-epoch:506b', 'late.pdf', 2);
  doc.getElementById('ceremonyConveneBtn').click();
  await settle();
  assert.match(line506().textContent, /late\.pdf/, 'setup: the fresh open did not bind late.pdf');
  // A roster and a deadline, so the press reaches the binding check rather than an earlier refusal.
  const box = doc.querySelector('#cerPeerPick .cerpeerbox');
  assert.ok(box, 'setup: the peer picker has no rows');
  box.checked = true;
  const exp = doc.getElementById('cerExpires');
  if (exp) exp.value = '2027-01-01T12:00';

  await closeTabNamed('late.pdf'); // the strip is live behind the sheet
  const convenes = calls.filter((c) => c.url.includes('/api/ceremony/convene')).length;
  doc.getElementById('ceremonyConveneForm').requestSubmit();
  await settle();
  assert.equal(calls.filter((c) => c.url.includes('/api/ceremony/convene')).length, convenes,
    'Convene posted with a pin to a closed document (a 409) — or with none, which the server answers with whatever is active');
  assert.match(doc.getElementById('cerConveneError').textContent, /no longer open/,
    'the press did nothing visible — the user is not told the document went away');
  assert.doesNotMatch(line506().textContent, /late\.pdf/, 'the sheet still names the closed document');
  doc.getElementById('cerConveneCancel').click();
  await settle();
});
