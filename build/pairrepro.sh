...
# complete a ceremony between them.
#
...
echo "initiating the ceremony…"
INIT_OUT="$WORK/initiate.json"
curl -sS -X POST "$A/api/session/initiate" -H "X-CSRF-Token: $CSRF_A" \
  ...
  -F "address=127.0.0.1:$SESSION_PORT" \
...
[ -n "$WORDS_A" ] || fail "instance A was never shown the verification words — the ceremony reached the document exchange without the spoken check (L2)"
[ -n "$WORDS_B" ] || fail "instance B was never shown the verification words"
[ "$(echo "$WORDS_A" | wc -w)" = 4 ] || fail "A's verification string is not four words: $WORDS_A"
# The property only two instances can see.
[ "$WORDS_A" = "$WORDS_B" ] || fail "the two instances derived DIFFERENT verification words — A saw '$WORDS_A' and B saw '$WORDS_B'. Two people comparing these would read a mismatch as an attack."

[ "$CODE" = "200" ] || { cat "$INIT_OUT" >&2; cat "$WORK/b/nib.log" >&2; fail "initiate returned HTTP $CODE"; }
...
curl -fsS "$B/api/pdf" -o "$WORK/final.pdf" || fail "could not fetch B's document"
python3 - "$WORK/final.pdf" <<'PY' || exit 1
import sys
b = open(sys.argv[1], "rb").read()
n = b.count(b"/ByteRange")
if n < 2:
    print(f"FAIL: the co-signed document carries {n} signature byte-ranges, want 2 — "
          f"the ceremony did not produce a document signed by both parties", file=sys.stderr)
    sys.exit(1)
print(f"the finished document carries {n} signatures")
PY

echo "PASS: a ceremony completed between two instances (words: $WORDS_A)"
