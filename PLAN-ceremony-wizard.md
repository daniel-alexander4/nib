...
### P04 — Scale and repair *(**done** 2026-09-09, v1.128.54 — four slices. **Acceptance ledger: 3 criteria, 6 clauses, all met** — `ledgers/2026-09-09-p04-acceptance.md`. **C01d was met by NOTHING when the last slice closed**: the rail's two renderings are driven either side of the boundary with `worklist` STUBBED, which asserts the panel and says nothing about the threshold — a wrong comparison renders perfectly on both sides of whatever number it uses. Built here, two mutations red. **That is the second consecutive phase close where the missing clause was the second half of an `and`** (P03's was *"survives closing AND reopening"*), which is the whole mechanism earning its keep twice. **The threshold was not chosen but CHECKED**: `SittingCeiling` already existed as *"what the UI should be designed and copy-written for"*, and S01 measured six roster sizes × two fixtures at a reference window the app had never declared — bracket 4 to 16–24, so 8 holds. **The full-repo review found six defects across four reviewed slices, and the worst was the one this phase's own grill pin was written to prevent**: the worklist rendered BELOW the roster instead of replacing it, so a 32-party card went from 32 rows to 59 — measured — while the shipped function's own comment claimed *"the summary replaces the roster"*. Its tier-2 fixture could not see it, because the card's roster was 2 parties and the worklist's was 13. Also fixed: `!c.ended` read as *"still running"* where `Stored.Ended`'s own doc forbids it by name, so a ceremony past its deadline minted a working invitation; S04's statement firing on two flows that sign nothing; an inert `isMe`; an unnameable convener; and three re-issue gates that went red nowhere. **Required-run gates**: tiers 0–3 ✓ (261 jsdom, 107 browser), vet ✓, gofmt ✓, tier 6 ✓ 19/19 and tier 4 ✓ both transports — **fired at S02 and S03**, and recorded as NOT firing at S01 and S04. **Graduation pass: 22 rows, all readers resolved, 0 hot-path** — checked because the first design put per-party progress on a request path the client calls on every mode switch, and the measurement moved it off. **Closure sweep: the `Phase` section is EMPTY**; `/pending 428` amended, because S04's shipped sentence now depends on no abandon existing and a guard asserts the word is absent.)*
