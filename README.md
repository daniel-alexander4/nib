...
**This is a checker, not a certificate.** Nib checks 104 of the 106 rules the reference validator
evaluates, so a document can pass every clause Nib checks and still fail one it does not. None is measured
today — veraPDF passes all 131 files of its own PDF/UA-1 test corpus that Nib calls conformant — because the
one unchecked rule a file can fail, a Type 1 font's CharSet, comes with an embedded font whose other rules Nib
still reports as "could not check". Nib's verdicts are tested, not proven, and that is also why Nib never
